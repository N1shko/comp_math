...
def calc_interpolation(inter, function, time_integr, h_integr, integr_eps, m, p_degree, eq_amount):
    initial_descrete = np.array([0.8, 0.6])
    tool = CalcTools()
...
def optimize_intervals(function, eq_amount, time_integr, h_integr, integr_eps, steps=None, p_degree=3, koeff_intervals=[], initial_intervals=[], initial_discrete=None):
    ...
    t, min_val, max_val = calc_interpolation(koeff_intervals, function, time_integr, h_integr, integr_eps, m, p_degree, eq_amount)
        ...
        for iterations in range(num_determine):
            ...
            for it in range(m):
                for k in range(2):
                    ...
                    t, new_min_val, new_max_val = calc_interpolation(new_interval, function, time_integr, h_integr, integr_eps, m, p_degree, eq_amount)
                    if iterations == num_determine-1 or iterations == 0:
                        fig, ax = plt.subplots(m, 1, figsize=(10, 10))
                        for graphic in range(m):
                            ax[graphic].grid()
...
if __name__ == '__main__':
    ...
    optimize_intervals(koeff_intervals=koeff_intervals, eq_amount=2, function=f, time_integr=5.5, h_integr=0.5, integr_eps=1e-3, steps=steps, p_degree=4,
                       initial_discrete=np.array([x_0, y_0]))